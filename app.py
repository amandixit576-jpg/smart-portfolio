...
if st.session_state.current_view == "COMPARE":
    ...
    if st.button("Run Comparison 🚀", type="primary"):
        ...
        comp_data = {
            ...
            TOP_STOCKS[t1]: [
                format_inr(i1.get('currentPrice')), round(i1.get('trailingPE', 0), 2) if i1.get('trailingPE') else 'N/A', 
                round(i1.get('priceToBook', 0), 2) if i1.get('priceToBook') else 'N/A', round(i1.get('returnOnEquity', 0)*100, 2) if i1.get('returnOnEquity') else 'N/A', 
                round(i1.get('debtToEquity', 0), 2) if i1.get('debtToEquity') else 'N/A', f"₹{format_large_number(i1.get('marketCap'))}"
            ],
            TOP_STOCKS[t2]: [
                format_inr(i2.get('currentPrice')), round(i2.get('trailingPE', 0), 2) if i2.get('trailingPE') else 'N/A', 
                round(i2.get('priceToBook', 0), 2) if i2.get('priceToBook') else 'N/A', round(i2.get('returnOnEquity', 0)*100, 2) if i2.get('returnOnEquity') else 'N/A', 
                round(i2.get('debtToEquity', 0), 2) if i2.get('debtToEquity') else 'N/A', f"₹{format_large_number(i2.get('marketCap'))}"
            ]
        }
...
# --- 4.5 MUTUAL FUND TRACKER ---
    ...
    MF_DICT = {
        "Parag Parikh Flexi Cap Fund": "0P0000YWL1.BO", "SBI Small Cap Fund": "0P0000XW8F.BO", "Axis Bluechip Fund": "0P0000XVAA.BO",
        "HDFC Mid-Cap Opportunities": "0P0000XVU0.BO", "ICICI Prudential Technology Fund": "0P0000XVYV.BO", "Quant Small Cap Fund": "0P0001B61B.BO"
    }
    ...
    if st.button("Fetch NAV & Chart 🚀", type="primary"):
        with st.spinner("Fetching latest NAV..."):
            ...
            try: hist = mf_data.history(period="1y")
...
# --- 4.6 THEME BASKETS ---
    ...
    with b1:
        ...
        if st.button("Analyze Green Energy", key="b_green", use_container_width=True): st.session_state.current_view = "TATAPOWER.NS"; st.rerun()
    with b2:
        ...
        if st.button("Analyze Monopolies", key="b_mono", use_container_width=True): st.session_state.current_view = "IRCTC.NS"; st.rerun()
    with b3:
        ...
        if st.button("Analyze PSU Banks", key="b_psu", use_container_width=True): st.session_state.current_view = "SBIN.NS"; st.rerun()
    st.write("")
    ...
    with b5:
        ...
        if st.button("Analyze Tech Basket", key="b_tech", use_container_width=True): st.session_state.current_view = "HCLTECH.NS"; st.rerun()
    with b6:
...
# --- MEGA FOOTER (DETAILED ABOUT & SOCIAL ICONS) ---
mega_footer = """
...
.footer-brand {
    flex: 1.6; /* About section ko thodi zyada jagah dene ke liye */
    padding-right: 40px;
}
.footer-col h4 {
    color: #E2E8F0;
...
.social-icons a:hover {
    ...
    transform: translateY(-3px); /* Halka sa upar uthne ka effect */
}
    ...
    <div class="footer-grid">
        <div class="footer-col footer-brand">
            ...
            <p style="font-size: 13px; line-height: 1.7; text-align: justify;">
                Dixit Investment Group (DIG) is a premier quantitative research and portfolio management terminal designed for modern value investors and financial professionals. Built on the principles of deep fundamental analysis and data-driven investing, DIG provides institutional-grade market screeners, real-time SIP planning, and comprehensive tax audit tools. Our mission is to democratize elite financial analytics, empowering retail investors to make calculated, risk-adjusted decisions in the Indian equity markets.
            </p>
